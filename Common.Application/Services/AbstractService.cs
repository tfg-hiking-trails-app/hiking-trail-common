    ...
    : IService<TEntityDto, TCreateEntityDto, TUpdateEntityDto> where TEntity : BaseEntity
{
    ...
    {
        TEntity entity = await GetEntityAsync(code);
    ...
    protected TEntity GetEntity(Guid code)
    {
        if (code == Guid.Empty)
            throw new ArgumentNullException(nameof(code));
        
        TEntity? entity = Repository.GetByCode(code);
    ...
    protected async Task<TEntity> GetEntityAsync(Guid code)
    {
        if (code == Guid.Empty)
            throw new ArgumentNullException(nameof(code));
